...
this.ovmap.events.register("addlayer", this, function (e) {
...
Zondy.Map.TianDiTuLayer = OpenLayers.Class(OpenLayers.Layer.Grid, {
    ...
    */
    ...
    /*注意：调用clone的方式时，当zoomOffset为0时，该方法中会将此参数强制设置为1，
    主要是为解决添加鹰眼时，鹰眼中的默认地图可以显示（造成此问题的原因是天地图默认第0级没有图）*/
    clone: function (obj) {
        /// <summary>克隆此图层</summary>
        if (obj == null) {
            /*modify by zl 2016-07-27 强制设置zoomOffset的值，
            当zoomOffset为0时，设置此参数为1，以次来解决在鹰眼中不显示的问题*/
            var opt = this.getOptions();
            if (this.opt_option!=null&&this.zoomOffset==0) {
                this.opt_option.zoomOffset = 1;
            }
            opt = OpenLayers.Util.extend(opt, this.opt_option);
            /*将OpenLayers.Layer.TileLayer改为Zondy.Map.TianDiTuLayer
            *modify moveBy liuruoli 2015-11-11 */
            obj = new Zondy.Map.TianDiTuLayer(this.name,opt);
        } else {
            obj = OpenLayers.Layer.Grid.prototype.clone.apply(this, [obj]);
        }
        return obj;
...
Zondy.Enum.TianDiTu = {
    ...
    Raster: 'img',
    Annotation: 'cva',
    RasterAnnotation: 'cia',
    Vecotr_IGS: 'vector_igs', //天地图矢量数据(通过IGS)
    Raster_IGS: 'raster_igs', //天地图影像数据(通过IGS)
    Annotation_IGS: 'vectorAnnotation_igs', //天地图矢量注记数据(通过IGS)
    RasterAnnotation_IGS: 'rasterAnnotation_igs'  //天地图影像注记数据(通过IGS)
};
Zondy.Map.TianDiTu = OpenLayers.Class(Zondy.Map.TianDiTuLayer, {
    zoomOffset: 0,
    //天地图类型
    ...
    initialize: function (name, options) {
        ...
        var newArguments = [];
        newArguments.push(name, {}, {});
        switch (this.layerType.toString()) {
            case "0":
                this.layerType = Zondy.Enum.TiandituType.VEC;
                break;
            case "1":
                this.layerType = Zondy.Enum.TiandituType.IMG;
                break;
            case "2":
                this.layerType = Zondy.Enum.TiandituType.CVA;
                break;
            case "vec":
                this.layerType = Zondy.Enum.TiandituType.VEC;
                break;
            ...
            case "cva":
                ...
                break;
            case "cia":
                this.layerType = Zondy.Enum.TiandituType.CIA;
                break;
            case "vector_igs":
                this.layerType = Zondy.Enum.TiandituType.VEC_IGS;
                break;
            case "raster_igs":
                this.layerType = Zondy.Enum.TiandituType.IMG_IGS;
                break;
            case "vectorAnnotation_igs":
                this.layerType = Zondy.Enum.TiandituType.CVA_IGS;
                break;
            case "rasterAnnotation_igs":
                this.layerType = Zondy.Enum.TiandituType.CIA_IGS;
                break;
        }
        this.opt = options;
        Zondy.Map.TianDiTuLayer.prototype.initialize.apply(this, newArguments);
    },

    destroy: function () {
        /// <summary>销毁此图层</summary>
        // for now, nothing special to do here. 
        Zondy.Map.TianDiTuLayer.prototype.destroy.apply(this, arguments);
    },
    /*注意：调用clone的方式时，当zoomOffset为0时，该方法中会将此参数强制设置为1，
    主要是为解决添加鹰眼时，鹰眼中的默认地图可以显示（造成此问题的原因是天地图默认第0级没有图）*/
    clone: function (obj) {
        ...
        if (obj == null) {
            ...
            当zoomOffset为0时，设置此参数为1，以次来解决在鹰眼中不显示的问题*/
            var opt = this.getOptions();
            if (this.opt && this.zoomOffset == 0) {
                this.opt.zoomOffset = 1;
            }
            opt = OpenLayers.Util.extend(opt, this.opt);
            obj = new Zondy.Map.TianDiTu(this.name, opt);
        } else {
            obj = OpenLayers.Layer.Grid.prototype.clone.apply(this, [obj]);
        }
        return obj;
...
Zondy.Object.CDynNoteInfo = OpenLayers.Class({
    ...
    /// <summary>字形{Zondy.Enum.FontShape}</summary>
    Ifnx: Zondy.Enum.FontShape.Positive,
    ...
    /// <summary>注记冲突策略</summary>
    LabelLevel: 0,

    /// <summary>注记方位属性{Zondy.Object.DynNoteLabelType}</summary>
    LabelType: null,
...
Zondy.Service.AnalysisBase = OpenLayers.Class(Zondy.Service.HttpRequest, {
    ...
    execute: function (onSuccess, way, isAsy, f,onError) {
        ...
        this.ajax(null, data, onSuccess, way, null, f,onError);
    }
});
/*----------------------------------------------------------------------------------------------------------------------------*/
...
Zondy.Service.ProjectDots = OpenLayers.Class(Zondy.Service.GeometryAnalysisBase, {
    ...
    */
    execute: function (onSuccess, onError) {
        ...
        this.ajax(null, jsonString, onSuccess, "POST",null,null,onError);
    }
});
/*----------------------------------------------------------------------------------------------------------------------------*/
...
Zondy.Service.TopAnalysis = OpenLayers.Class(Zondy.Service.GeometryAnalysisBase, {
    ...
    * type-{string}请求类型,取值"Get","Post"，当前功能默认只取值POST，设置GET无效，只做兼容保留
    * contentType-{string}get方式默认为‘application/x-www-form-urlencoded’，post默认为text/plain,一般使用默认即可
    * options-{Object} 用来扩展ajax的参数，一般无需设置
    */
    execute: function (onSuccess,onError) {
        ...
        var postString = $.toJSON(postObj);
        this.ajax(null, postString, this.onGetRltSuccess, "POST",null,null,onError);
    },
...
Zondy.Service.NetAnalysisExtent = OpenLayers.Class(Zondy.Service.AnalysisBase, {
    ...
    */
    getNetInfo: function (infoType, onSuccess, onError) {
    ...
    * options-{Object} 主要用来扩展ajax的参数，一般无需设置
    */
    addNetFlag: function (dotVal, onSuccess, onError, type) {
        var postString = null;
        ...
        this.ajax(null, postString, onSuccess, type, null, null, onError);
    },
    /** 
    * 执行网络分析（只支持POST方式）
    * Parameters:
    ...
    * options-{Object} 主要用来扩展ajax的参数，一般无需设置
    */
    netAnalyse: function (dataObject, onSuccess, onError) {
        this.baseUrl = "igs/rest/netAnaly";
        this.partUrl = "netAnalyse";
        //当前服务只支持POST的方式
        this.ajax(null, dataObject, onSuccess, "POST", null, null, onError);
    },
    /** 
    * 执行多策略网络分析（只支持POST方式）
    * Parameters:
    ...
    * options-{Object} 主要用来扩展ajax的参数，一般无需设置
    */
    comNetAnalyse: function (dataObject, onSuccess, onError) {
        this.baseUrl = "igs/rest/netAnaly";
    ...
    * options-{Object} 主要用来扩展ajax的参数，一般无需设置
    */
    pluNetAnalyse: function (dataObject, onSuccess, onError) {
        this.baseUrl = "igs/rest/netAnaly";
...
Zondy.Service.Catalog.GDBInfo = OpenLayers.Class(Zondy.Service.Catalog.CatalogServiceBase, {
    ...
    getServerList: function (onSuccess, onError) {
        /// <summary>获取数据源列表</summary>
        /// <param name="onSuccess" type="Function">回调函数</param>
        this.partUrl = "datasource?f=" + this.resultFormat;
        this.ajax(null, null, onSuccess, "GET", null, null, onError)
    },
    getGDBList: function (onSuccess, onError) {
    ...
    getDsList: function (onSuccess,onError) {
        ...
        this.ajax(null, null, onSuccess, "GET", null, null, onError, { async: false });
    },

    getRcsList: function (onSuccess, onError) {
    ...
    AttachGDB: function (path, onSuccess, onError) {
    ...
    DetachGDB: function (onSuccess,onError) {
    ...
    CreateGDB: function (path, onSuccess, onError) {
    ...
    DeleteGDB: function (onSuccess, onError) {
...
Zondy.Service.Catalog.MapDoc = OpenLayers.Class(Zondy.Service.Catalog.CatalogServiceBase, {
    ...
    */
    ...
    getMapDocList: function (onSuccess,onError) {
    ...
    getMapDocInfo: function (onSuccess, details, sub, onError) {
    ...
    getMapInfo: function (onSuccess, returnFullStyle, details, onError) {
        /// <summary>获取指定地图的相关信息</summary>
        /// <param name="returnFullStyle" type="Boolean">返回结果是否包含细节内容</param>
        /// <param name="details" type="Boolean">兼容旧版本，该参数无效，调用时可不设置</param>
    ...
    getLayersInfo: function (onSuccess, onError) {
    ...
    getLayerInfo: function (onSuccess, onError) {
    ...
    deleteLayer: function (onSuccess, onError) {
    ...
    addLayer: function (addLayerInfos, onSuccess, onError) {
        ...
        this.ajax(null, addLayerInfos, onSuccess, "POST", null, null, onError);
    },

    changeIndex: function (newIndexArray, onSuccess, onError) {
        ...
        this.ajax(null, newIndexArray, onSuccess, "POST", null, null, onError);
    },
    getLegendInfo: function (layerIDs, fields, onSuccess, onError) {
...
Zondy.Service.Catalog.TileLayer = OpenLayers.Class(Zondy.Service.Catalog.CatalogServiceBase, {
    ...
    getTileList: function (onSuccess, onError) {
    ...
    getTileInfo: function (onSuccess, onError) {
...
Zondy.Service.Catalog.VectorLayer = OpenLayers.Class(Zondy.Service.Catalog.GDBInfo, {
    ...
    getSfclsList: function (onSuccess,onError) {
        ...
        this.ajax(null, null, onSuccess, null, null, null, onError, { async: false });
    },
    getAclsList: function (onSuccess,onError) {
    ...
    getOclsList: function (onSuccess,onError) {
    ...
    getNclsList: function (onSuccess,onError) {
    ...
    getRdsList: function (onSuccess, onError) {
    ...
    getDsSfclsList: function (onSuccess, onError) {
    ...
    getDsAclsList: function (onSuccess, onError) {
    ...
    getDsOclsList: function (onSuccess, onError) {
    ...
    getDsNclsList: function (onSuccess, onError) {
    ...
    getRdsListInRcs: function (onSuccess, onError) {
    ...
    getLayerList: function (clsType, onSuccess, onError) {
    ...
    getLayerListInDS: function (clsType, onSuccess, onError) {
    ...
    getLayerInfo: function (gdbpUrl, onSuccess, onError) {
    ...
    CreateVectCls: function (vectCls, onSuccess, onError) {
        ...
        this.partUrl += this.serverName + "/" + this.gdbName + "/" + vectCls.clsType + "/" + vectCls.clsName + "/create?"
          ...
          this.ajax(null, attStructStr, onSuccess, "POST", null, this.resultFormat, onError);
    },
      deleteXCls: function (clsType, clsName, onSuccess, onError) {
...
Zondy.Service.Catalog.ColorInfo = OpenLayers.Class(Zondy.Service.Catalog.CatalogServiceBase, {
    ...
    getColorNO: function (options, onSuccess, onError) {
    ...
    getColorRGB: function (options, onSuccess, onError) {
...
//        this.mapIndex = mapIndex;
    ...
    add: function (features, onSuccess, onError) {
        ...
        this.ajax(url, features, onSuccess, "POST", null, null, onError);
    },

    update: function (features, onSuccess, onError) {
    ...
    deletes: function (featureIds, onSuccess, onError) {
        ...
        this.ajax(url, null, onSuccess, "POST", null, null, onError);
    }
});
/*----------------------------------------------------------------------------------------------------------------------------*/
...
Zondy.Service.QueryServiceBase = OpenLayers.Class(Zondy.Service.HttpRequest, {
    ...
    */
    queryParam: null,
    /**
    *  定义查询的方式是GET还是POST，目前查询只有基于矢量图层的支持GET和POST，矢量地图文档的暂只支持GET方式
    *  {Zondy.Service.QueryParameter}
    */
    requestType: "GET",
    ...
    query: function (onSuccess, onError, requestType) {
        /// <summary>查询函数，向服务器发送请求</summary>
        /// <param name="onSuccess" type="Function">回调函数</param>
        /// <param name="onError" type="Function">请求失败回调函数</param>
        /// <param name="requestType" type="string">请求方式：GET或者POST</param>
        ...
        dataObject.rlt = Math.random();
        this.restQuery(fullRestUrl, dataObject, onSuccess, way, onError);
    },

    restQuery: function (restUrl, dataObject, onSuccess, way, onError) {
        this.resultCallBack = onSuccess;
        this.ajax(restUrl, dataObject, this.processResult, way, null, dataObject.f, onError);
    }
});
/*----------------------------------------------------------------------------------------------------------------------------*/
...
Zondy.Service.ThemeOper = OpenLayers.Class({
    ...
    */
    getThemesInfo: function (mapDocName, idxArr, onSuccess,onError) {
    ...
    * onSuccess(themesInfoArr) : 获取成功回调方法
    */
    removeThemesInfo: function (mapDocName, idxArr, onSuccess, onError) {
        var rand = Math.random();
    ...
    * onSuccess(themesInfoArr) : 获取成功回调方法
    */
    updateThemesInfo: function (mapDocName, idxArr, themesInfoArr, onSuccess, onError) {
        ...
        if (jsStr != null) {
            ...
            }, "POST", "application/text", null, onError);
        }
        else {
            onSuccess(false);
        }
    ...
    * onSuccess(themesInfoArr) : 获取成功回调方法
    */
    addThemesInfo: function (mapDocName, idxArr, themesInfoArr, onSuccess, onError) {
...
Zondy.Service.MultiGeoQuery = OpenLayers.Class(Zondy.Service.QueryServiceBase, Zondy.Service.DocLayer, {
    ...
    query: function (onSuccess, onError) {
        /// <summary>查询函数，向服务器发送请求</summary>
        /// <param name="onSuccess" type="Function">回调函数</param>
        if (this.queryParam == null) {
            return;
        ...
        this.ajax(fullRestUrl, dataObj, onSuccess, "post", null, this.resultFormat, onError);
    }
});
/*-------------------------------------------------------------------------------------------------------------------------------------------*/
...
Zondy.Service.ObjClsQuery = OpenLayers.Class(Zondy.Service.QueryServiceBase, {
    ...
    query: function (onSuccess,onError) {
        ...
        this.ajax(fullRestUrl, null, onSuccess, "GET", null, this.resultFormat, onError);
    }
});
/*-------------------------------------------------------------------------------------------------------------------------------------------*/
...
Zondy.Layer = OpenLayers.Class(Zondy, {
});
Zondy.Marker = OpenLayers.Class(Zondy, {
});
/**
* @classdesc
* 扩展的Markers的图层，继承类OpenLayers.Layer.Markers
* 重写了drawMarker的方法，可以将div的标注添加到中心点的位置
* @api
*/
Zondy.Layer.HtmlMarkers = OpenLayers.Class(OpenLayers.Layer.Markers, {
    /**
    * Method: drawMarker 
    ...
    *
    * Parameters: 
    * marker - {<Zondy.Marker.HtmlMarker>} 
    */
    drawMarker: function (marker) {
        var centerPx = this.map.getLayerPxFromLonLat(marker.lonlat);
        var sz = marker.size;
        var offset = marker.offset;
        var topleft = new OpenLayers.Pixel(centerPx.x - sz.w / 2 - offset.x, centerPx.y - sz.h / 2 - offset.y);
    ...
    */
    ...
    CLASS_NAME: "Zondy.Layer.HtmlMarkers"
});
...
/**
* Class: Zondy.Marker.HtmlMarker
* 继承OpenLayers.Marker，以便可以直接传入点坐标，添加div的Marker
* Inherits from:
*  - <OpenLayers.Marker> 
*/
Zondy.Marker.HtmlMarker = OpenLayers.Class(OpenLayers.Marker, {
    ...
    * borderWidth - {int} 
    */
    initialize: function (lonlat, size, offset, options) {
        this.lonlat = lonlat;
        if (size != null) {
            this.size = size;
        }
    ...
    */
    ...
    CLASS_NAME: "Zondy.Marker.HtmlMarker"
});
...
Zondy.Object.ContourAnalyse.MeshingParam = function (opt_options) {
    ...
    this.SfClsURL = options.SfClsURL != undefined ? opt_options.SfClsURL : null;
    this.FieldName = options.FieldName != undefined ? opt_options.FieldName : null;
    this.XCellNum = options.XCellNum != undefined ? opt_options.XCellNum : 200;
    this.Bound = options.Bound != undefined ? opt_options.Bound : null;
};
/**
*	平面等值线追踪所用到的注记参数类
...
Zondy.Object.ContourAnalyse.ContourNoteParam = function (opt_options) {
    var options = (opt_options != undefined) ? opt_options : {};
    this.IsClipLine = options.IsClipLine != undefined ? opt_options.IsClipLine : true;
    this.isXYScaleOut = options.isXYScaleOut != undefined ? opt_options.isXYScaleOut : false;
    this.NoteDirection = options.NoteDirection != undefined ? opt_options.NoteDirection : 1;
    this.LineWidth = options.LineWidth != undefined ? opt_options.LineWidth : 0.05;
    this.MaxAngle = options.MaxAngle != undefined ? opt_options.MaxAngle : 90.0;
    this.MinDist = options.MinDist != undefined ? opt_options.MinDist : 10.0;
    this.IsAbs = options.IsAbs != undefined ? opt_options.IsAbs : false;
    this.IsComma = options.IsComma != undefined ? opt_options.IsComma : false;
    this.DigitNum = options.DigitNum != undefined ? opt_options.DigitNum : 0;
    this.FormatNo = options.FormatNo != undefined ? opt_options.FormatNo : 0;
    this.LogFlag = options.LogFlag != undefined ? opt_options.LogFlag : 0;
    this.Prefix = options.Prefix != undefined ? opt_options.Prefix : "";
    this.Suffix = options.Suffix != undefined ? opt_options.Suffix : "";
    this.ColorNo = options.ColorNo != undefined ? opt_options.ColorNo : 0;
    this.FixSize = options.FixSize != undefined ? opt_options.FixSize : 0.01;
    this.FontNo = options.FontNo != undefined ? opt_options.FontNo : 0;
};
/*
*	等值线层参数类，用来描述每一层的信息
...
Zondy.Object.ContourAnalyse.ContourZValue = function (opt_options) {
    var options = (opt_options != undefined) ? opt_options : {};
    this.ZValue = options.ZValue != undefined ? opt_options.ZValue : 1;
    this.LineInfo = options.LineInfo != undefined ? opt_options.LineInfo : null;
    this.RegInfo = options.RegInfo != undefined ? opt_options.RegInfo : null;
    this.IsOutputNote = options.IsOutputNote != undefined ? opt_options.IsOutputNote : false;
};
/*
*	描述线参数
...
Zondy.Object.ContourAnalyse.CLineInfo = function (opt_options) {
    var options = (opt_options != undefined) ? opt_options : {};
    this.LinWidth = options.LinWidth != undefined ? opt_options.LinWidth : 1;
    this.Color = options.Color != undefined ? opt_options.Color : 1;
    this.LinStyleID = options.LinStyleID != undefined ? opt_options.LinStyleID : 1;
    this.LinStyleID2 = options.LinStyleID2 != undefined ? opt_options.LinStyleID2 : 0;
    this.Xscale = options.Xscale != undefined ? opt_options.Xscale : 1.0;
    this.Yscale = options.Yscale != undefined ? opt_options.Yscale : 1.0;
};
/*
*	描述区参数
...
Zondy.Object.ContourAnalyse.CRegionInfo = function (opt_options) {
    var options = (opt_options != undefined) ? opt_options : {};
    this.PatID = options.PatID != undefined ? opt_options.PatID : 0;
    this.FillMode = options.FillMode != undefined ? opt_options.FillMode : 0;
    this.FillColor = options.FillColor != undefined ? opt_options.FillColor : 1;
    this.PatHeight = options.PatHeight != undefined ? opt_options.PatHeight : 1.0;
    this.PatWidth = options.PatWidth != undefined ? opt_options.PatWidth : 1.0;
    this.PatAngle = options.PatAngle != undefined ? opt_options.PatAngle : 1.0;
    this.PatColor = options.PatColor != undefined ? opt_options.PatColor : 1.0;
    this.OutPenWidth = options.OutPenWidth != undefined ? opt_options.OutPenWidth : 1.0;
    this.OverMethod = options.OverMethod != undefined ? opt_options.OverMethod : 0;
};
/*
*	示坡线参数类
...
Zondy.Object.ContourAnalyse.SlopLineParam = function (opt_options) {
    var options = (opt_options != undefined) ? opt_options : {};
    this.XScale = options.XScale != undefined ? opt_options.XScale : 2.0;
    this.YScale = options.YScale != undefined ? opt_options.YScale : 10.0;
    this.LineType = options.LineType != undefined ? opt_options.LineType : 0;
    this.SubLineType = options.SubLineType != undefined ? opt_options.SubLineType : 0;
};
/*
*	平面等值线追踪参数类
...
Zondy.Object.ContourAnalyse.ContourParam = function (opt_options) {
    var options = (opt_options != undefined) ? opt_options : {};
    this.IsSmoothLine = options.IsSmoothLine != undefined ? opt_options.IsSmoothLine : false;
    this.SmoothGrade = options.SmoothGrade != undefined ? opt_options.SmoothGrade : 1;
    this.IsMakeReg = options.IsMakeReg != undefined ? opt_options.IsMakeReg : false;
    this.IsMakeNote = options.IsMakeNote != undefined ? opt_options.IsMakeNote : false;
    this.IsMakeSLin = options.IsMakeSLin != undefined ? opt_options.IsMakeSLin : false;
    this.MapWay = options.MapWay != undefined ? opt_options.MapWay : 1;
    this.FrameWidth = options.FrameWidth != undefined ? opt_options.FrameWidth : 1.0;
    this.FrameHeight = options.FrameHeight != undefined ? opt_options.FrameHeight : 1.0;
    this.IsDrawColorScl = options.IsDrawColorScl != undefined ? opt_options.IsDrawColorScl : false;
    this.IsSaveEdge = options.IsSaveEdge != undefined ? opt_options.IsSaveEdge : false;
    this.NoteParam = options.NoteParam != undefined ? opt_options.NoteParam : null;
    this.SlopLineParam = options.SlopLineParam != undefined ? opt_options.SlopLineParam : null;
    this.ZValues = options.ZValues != undefined ? opt_options.ZValues : null;
};
